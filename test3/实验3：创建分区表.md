...
在lh用户中创建两个表：orders（订单表）和order_details（订单详表），两个表通过列order_id建立主外键关联。orders表按范围分区进行存储，order_details使用引用分区进行存储。
**创建orders表的部分语句是：**

```sql
SQL>CREATE TABLESPACE users02 DATAFILE
'/home/student/lh/pdbtest_users02_1.dbf'
  SIZE 100M AUTOEXTEND ON NEXT 50M MAXSIZE UNLIMITED，
'/home/student/lh/pdbtest_users02_2.dbf' 
  SIZE 100M AUTOEXTEND ON NEXT 50M MAXSIZE UNLIMITED
...
select * from lh.orders where order_date
...
from lh.orders a,lh.order_details b where
